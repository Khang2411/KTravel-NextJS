...
export const HostingList = ({ rooms }: HostingListProps) => {
  ...
  const rows = observer.map((room) =>
    createData(
      room.id,
      room?.images[0]?.image ? room.images[0].image : "",
      room.name
        ? room.name
        : "Được khởi tạo vào " + format(parseISO(room.updated_at), "dd/MM/Y"),
      room.user.verify_account !== 1
        ? "Yêu cầu xác minh"
        : room.status === null || room.status === "processing"
        ? "Đang tiến hành"
        : "Hoàn thành",
      room.bedroom,
      room.bathroom,
  ...
  return (
    <>
      <Box sx={{ width: "100%" }}>
        <Box>
          {profile?.data.verify_account !== 1 && (
            <Typography>
              Tài khoản chưa xác minh{" "}
              <ErrorOutlineOutlinedIcon sx={{ color: "#f44336" }} />
            </Typography>
          )}
        </Box>
        <Box mb={2}>
                        ...
                        >
                          ...
                          >
                            <Box>
                              <Image
                                src={
                                  row.image? row.image: "https://a0.muscache.com/pictures/73709f4b-4e5e-4dc9-adc8-79f4df81ad3c.jpg"
                                }
                                // fill={true}
                                alt={"listing-image"}
                                width={100}
                                height={105}
                                style={{ objectFit:'cover', borderRadius: "8px",maxWidth:"105px", height:"105px" }}
                              />
                            </Box>
