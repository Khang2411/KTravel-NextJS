import {
  ...
  Price,
  PropertyRooms,
} from "@/components/manage-your-space";
...
const getRoom = async (id: string | number): Promise<Response<Room>> => {
  const res = await fetch(
    `${process.env.NEXT_PUBLIC_API_URL}/api/v1/user/rooms/${id}`,
    {
      headers: {
      ...
      <Stack direction={"row"} gap={8}>
        ...
        <Box>
          ...
          <Location
            ...
            longitude={room?.data.longitude}
            latitude={room?.data.latitude}
          />
          <Divider />
          <PropertyRooms
            id={params.id}
            privacy={room?.data.privacy}
            category={room?.data.category}
            adult={room?.data.adult}
            child={room?.data.child}
            bedroom={room?.data.bedroom}
            bathroom={room?.data.bathroom}
          ></PropertyRooms>
          <Divider />
          ...
          <Discount
            id={params.id}
            weekly_discount={room?.data.weekly_discount}
            monthly_discount={room?.data.monthly_discount}
            new_discount={room?.data.new_discount}
          ></Discount>
        </Box>
      </Stack>
    </Box>
